...
const Analytics = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axios.get(
        "https://langchain-backend.loca.lt/api/shipment-data"
      );
  ...
  const generateMockData = () => {
    ...
    for (let i = 0; i < 50; i++) {
      mockData.push({
        ...
        ShipmentCompart: shipmentComparts[Math.floor(Math.random() * shipmentComparts.length)],
        ExitTime_DayOfWeek: ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"][Math.floor(Math.random() * 5)],
  ...
  const getGraphicWalkerData = () => {
    ...
    const fields = [
      { fid: 'BayCode', name: 'Bay Code', semanticType: 'nominal', analyticType: 'dimension' },
      { fid: 'ShipmentCompart', name: 'Shipment Compartment', semanticType: 'nominal', analyticType: 'dimension' },
      { fid: 'BaseProductID', name: 'Base Product ID', semanticType: 'nominal', analyticType: 'dimension' },
      { fid: 'ShipmentID', name: 'Shipment ID', semanticType: 'nominal', analyticType: 'dimension' },
      { fid: 'ExitTime_DayOfWeek', name: 'Exit Day of Week', semanticType: 'nominal', analyticType: 'dimension' },
      { fid: 'GrossQuantity', name: 'Gross Quantity', semanticType: 'quantitative', analyticType: 'measure' },
      { fid: 'FlowRate', name: 'Flow Rate', semanticType: 'quantitative', analyticType: 'measure' },
      { fid: 'BaseProductCode', name: 'Base Product Code', semanticType: 'quantitative', analyticType: 'measure' },
      { fid: 'ShipmentCode', name: 'Shipment Code', semanticType: 'quantitative', analyticType: 'measure' },
      { fid: 'ExitTime', name: 'Exit Time', semanticType: 'temporal', analyticType: 'dimension' },
      { fid: 'ScheduledDate', name: 'Scheduled Date', semanticType: 'temporal', analyticType: 'dimension' },
    ];
  ...
  const generateCustomChart = () => {
    ...
    const spec = {
      ...
      encoding: {
        ...
        ...(selectedFields.color && {
          color: { field: selectedFields.color, type: "nominal" },
        }),
        ...(selectedFields.size && {
          size: { field: selectedFields.size, type: "quantitative" },
        }),
        ...(selectedFields.opacity && {
          opacity: { field: selectedFields.opacity, type: "quantitative" },
        }),
      },
      width: 400,
      height: 300,
    ...
    vegaEmbed(customChartRef.current, spec, { actions: false }).catch(
      console.error
    );
  };
  ...
  return (
    <main className="main-content">
      ...
      <div className="view-toggle">
        ...
        <button 
          className={`view-button ${activeView === 'graphicwalker' ? 'active' : ''}`}
      ...
      {activeView === 'charts' ? (
        ...
        <div className="graphicwalker-section">
          <div className="section-header">
            ...
            <p>Professional drag-and-drop analytics interface similar to Tableau</p>
          </div>
          {loading ? (
            <div className="chart-loading">Loading data for GraphicWalker...</div>
          ) : data.length > 0 ? (
